...
const JoinPoolContext = React.createContext({ openJoin: () => {} });

export default function Hero() {
  // Join modal state available to the whole page
  const [joinOpen, setJoinOpen] = React.useState(false);
  const [activePool, setActivePool] = React.useState(null);
  const openJoin = (pool) => { setActivePool(pool); setJoinOpen(true); };
...
// --- Join Modal (appears when a user clicks “Join”) ---
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 sm:p-6 md:p-8">
      ...
      <div className="relative w-full max-w-2xl rounded-2xl bg-white shadow-xl">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          ...
          <button onClick={onClose} className="h-9 w-9 grid place-items-center rounded-md hover:bg-slate-50">
            ✕
          </button>
        </div>
  ...
  return (
    <div className=\"rounded-2xl border border-slate-200 bg-white p-4 shadow-sm\">
      ...
      <div className=\"mt-4 grid grid-cols-2 gap-2\">
        <button className=\"rounded-lg px-3 py-2 font-medium text-white hover:opacity-90\" style={{ backgroundColor: colors.navy }} onClick={() => openJoin(pool)}>Join</button>
